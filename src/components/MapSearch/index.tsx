    ...
    >
      ...
      {markers.map(markerPoint => (
        <Marker
          ...
          icon={getCustomIconMarkerByKm(markerPoint.km, markerPoint.id === selected?.id)}
          eventHandlers={{
            click: () => onMarkerClick?.(markerPoint)
          }}
        >
          <Popup>
            <h1 className="text-lg font-normal text-red-700 py-1">
              {markerPoint.name}
            </h1>
            <h3 className="text-sm font-normal italic">
              {markerPoint.rod}
            </h3>
            <h3 className="text-sm font-light">{markerPoint.cityUf}</h3>
            <br />
            <strong>Coordenadas</strong>: <i className='text-indigo-700'>{markerPoint.position.join(', ')}</i> <br />
